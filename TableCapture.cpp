...
std::string TableCapture::saveSnapshot(const cv::Mat& fullTableImage, const std::string& folderPath, int shotNumber) const
{
    ...
    if (shotNumber >= 0)
    {
        oss << "coup" << std::setw(4) << std::setfill('0') << shotNumber << "_";
    }
