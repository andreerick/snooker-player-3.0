...
public:
    ...
    // Sauvegarde l'image complete sur disque, avec un nom horodate. Si
    // shotNumber >= 0, le numero du coup est inclus dans le nom du
    // fichier (meme convention que BallMapRecorder::saveSnapshot), pour
    // retrouver facilement la photo prise a un coup donne en cas de
    // contestation ou de miss a verifier. A appeler apres CHAQUE coup,
    // pas seulement en fin de frame : c'est la seule preuve visuelle
    // de l'etat reel de la table a cet instant.
    // Renvoie le chemin du fichier sauvegarde, ou une chaine vide en cas d'echec.
    std::string saveSnapshot(const cv::Mat& fullTableImage, const std::string& folderPath, int shotNumber = -1) const;
