...
public:
    ...
    // Repositionnement des billes (roadmap v1.0) : compare l'etat
    // ACTUEL (detecte en direct) a un etat CIBLE (recharge via
    // loadSnapshot(), par exemple le dernier coup valide avant une
    // contestation), et dessine un guide visuel :
    //   - contour pointille = position cible.
    //   - bille pleine + contour vert = deja a la bonne position.
    //   - bille pleine + contour blanc + fleche + distance (cm si
    //     pixelsPerCm > 0, sinon pixels) = a deplacer.
    //   - marqueur "MANQUANTE" = bille cible sans bille actuelle
    //     correspondante (bille absente de la table, a reposer).
    cv::Mat renderRepositioningGuide(
        const std::map<std::string, std::vector<TrackedBall>>& currentState,
