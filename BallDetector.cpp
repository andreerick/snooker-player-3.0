...
std::vector<DetectedBall> BallDetector::detectBallsCombined(
    const cv::Mat& frame,
    int minRadius,
    int maxRadius
) const
{
    std::vector<DetectedBall> combined = detectBalls(frame, minRadius, maxRadius);

    std::set<std::string> foundColors;
    for (const auto& ball : combined)
    {
        foundColors.insert(ball.colorName);
    }

    // Filet de securite par la forme, uniquement pour les couleurs
    // UNIQUES (une seule bille sur la table) qu'aucune bille couleur
    // n'a trouvees. On exclut "Rouge" expres : il y a jusqu'a 15 rouges
    // en meme temps, la teinte est deja tres fiable pour cette couleur,
    // et ajouter un filet par forme ici creerait des doublons difficiles
    // a demeler entre plusieurs rouges proches.
    std::vector<DetectedBall> byShape = detectBallsByShape(frame, minRadius, maxRadius);
    for (const auto& ball : byShape)
    {
        if (ball.colorName == "Rouge" || ball.colorName == "Inconnue")
        {
            continue;
        }
        if (foundColors.count(ball.colorName))
        {
            continue;
        }

        combined.push_back(ball);
        foundColors.insert(ball.colorName);
    }
