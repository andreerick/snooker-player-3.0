...
std::vector<RemovedBall> BallTracker::update(const std::vector<DetectedBall>& currentDetections)
{
    std::vector<RemovedBall> removed;
    ...
    for (auto& entry : m_tracked)
    {
        ...
        for (auto& prev : previousBalls)
        {
            ...
            {
                removed.push_back({ color, updated.lastPosition, updated.radius });
            }
        }
...
const std::map<std::string, std::vector<TrackedBall>>& BallTracker::getAllTracked() const
{
    return m_tracked;
}
